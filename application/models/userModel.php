...
/** inte 2 */
    ...
    public function membershipRequest()
    {
        ...
        $query=$this->db->query("SELECT credential.username, credential.email, credential.contact_number,membership.status
    FROM credential INNER JOIN membership ON credential.user_id=membership.user_id");
        return $query->result();
    }
    ...
    public function markAttendance()
    {
        ...
        $query=$this->db->query("select appointments.user_id,appointments.slot,appointments.created_at,credential.contact_number from appointments INNER JOIN credential ON credential.email=appointments.user_id where appointments.date='$selectedDate'");
        return $query->result();
    }
    ...
    {
        $query=$this->db->query("UPDATE appointments SET attendance = 1 WHERE user_id=$id");
        return $query->result();
